package br.com.sisgete.model;

/**
 *
 * @author luiz
 */
public class PacienteModel {

    private int idMedicamentoTratamento;
    private String acompanhamentoPaciente;
    private String alcool;
    private String alcoolObs;
    private String alcoolPeriodo;
    private String alcoolTipo;
    private String alergias;
    private String alergiasObs;
    private String alergiasPeriodo;
    private String alergiasTipo;
    private String atendente;
    private String bairro;
    private String cancer;
    private String cancerObs;
    private String cancerPeriodo;
    private String cancerTipo;
    private String cidade;
    private String conhecimentoEspiritaPrevio;
    private String dataAtendimento;
    private String dataNascimento;
    private String dengue;
    private String dengueObs;
    private String denguePeriodo;
    private String dengueTipo;
    private String diabetes;
    private String diabetesObs;
    private String diabetesPeriodo;
    private String diabetesTipo;
    private String doencaAutoimune;
    private String doencaAutoimuneObs;
    private String doencaAutoimunePeriodo;
    private String doencaAutoimuneTipo;
    private String doencaEstomago;
    private String doencaEstomagoObs;
    private String doencaEstomagoPeriodo;
    private String doencaEstomagoTipo;
    private String doencaOssos;
    private String doencaOssosObs;
    private String doencaOssosPeriodo;
    private String doencaOssosTipo;
    private String doencaPulmoes;
    private String doencaPulmoesObs;
    private String doencaPulmoesPeriodo;
    private String doencaPulmoesTipo;
    private String email;
    private String feridasTumores;
    private String feridasTumoresObs;
    private String feridasTumoresPeriodo;
    private String feridasTumoresTipo;
    private String fonteConhecimentoEspirita;
    private String fumo;
    private String fumoObs;
    private String fumoPeriodo;
    private String fumoTipo;
    private String hepatite;
    private String hepatiteObs;
    private String hepatitePeriodo;
    private String hepatiteTipo;
    private String hernia;
    private String herniaObs;
    private String herniaPeriodo;
    private String herniaTipo;
    private String hipertensao;
    private String hipertensaoObs;
    private String hipertensaoPeriodo;
    private String hipertensaoTipo;
    private String intensidadeSintomas;
    private String modoDesobsessao;
    private String nome;
    private String objetivoTratamento;
    private String pacienteReincidente;
    private String remedio;
    private String remedioObs;
    private String remedioPeriodo;
    private String remedioTipo;
    private String rua;
    private String setor;
    private String sintomas;
    private String statusTratamento;
    private String telefone;
    private String usoParacetamol;
    private String usoParacetamolObs;
    private String usoParacetamolPeriodo;
    private String usoParacetamolTipo;
    private int idade;
    private int numCasa;

    public int getIdMedicamentoTratamento() {
        return idMedicamentoTratamento;
    }

    public void setIdMedicamentoTratamento(int idMedicamentoTratamento) {
        this.idMedicamentoTratamento = idMedicamentoTratamento;
    }

    public String getAcompanhamentoPaciente() {
        return acompanhamentoPaciente;
    }

    public void setAcompanhamentoPaciente(String acompanhamentoPaciente) {
        this.acompanhamentoPaciente = acompanhamentoPaciente;
    }

    public String getAlcool() {
        return alcool;
    }

    public void setAlcool(String alcool) {
        this.alcool = alcool;
    }

    public String getAlcoolObs() {
        return alcoolObs;
    }

    public void setAlcoolObs(String alcoolObs) {
        this.alcoolObs = alcoolObs;
    }

    public String getAlcoolPeriodo() {
        return alcoolPeriodo;
    }

    public void setAlcoolPeriodo(String alcoolPeriodo) {
        this.alcoolPeriodo = alcoolPeriodo;
    }

    public String getAlcoolTipo() {
        return alcoolTipo;
    }

    public void setAlcoolTipo(String alcoolTipo) {
        this.alcoolTipo = alcoolTipo;
    }

    public String getAlergias() {
        return alergias;
    }

    public void setAlergias(String alergias) {
        this.alergias = alergias;
    }

    public String getAlergiasObs() {
        return alergiasObs;
    }

    public void setAlergiasObs(String alergiasObs) {
        this.alergiasObs = alergiasObs;
    }

    public String getAlergiasPeriodo() {
        return alergiasPeriodo;
    }

    public void setAlergiasPeriodo(String alergiasPeriodo) {
        this.alergiasPeriodo = alergiasPeriodo;
    }

    public String getAlergiasTipo() {
        return alergiasTipo;
    }

    public void setAlergiasTipo(String alergiasTipo) {
        this.alergiasTipo = alergiasTipo;
    }

    public String getAtendente() {
        return atendente;
    }

    public void setAtendente(String atendente) {
        this.atendente = atendente;
    }

    public String getBairro() {
        return bairro;
    }

    public void setBairro(String bairro) {
        this.bairro = bairro;
    }

    public String getCancer() {
        return cancer;
    }

    public void setCancer(String cancer) {
        this.cancer = cancer;
    }

    public String getCancerObs() {
        return cancerObs;
    }

    public void setCancerObs(String cancerObs) {
        this.cancerObs = cancerObs;
    }

    public String getCancerPeriodo() {
        return cancerPeriodo;
    }

    public void setCancerPeriodo(String cancerPeriodo) {
        this.cancerPeriodo = cancerPeriodo;
    }

    public String getCancerTipo() {
        return cancerTipo;
    }

    public void setCancerTipo(String cancerTipo) {
        this.cancerTipo = cancerTipo;
    }

    public String getCidade() {
        return cidade;
    }

    public void setCidade(String cidade) {
        this.cidade = cidade;
    }

    public String getConhecimentoEspiritaPrevio() {
        return conhecimentoEspiritaPrevio;
    }

    public void setConhecimentoEspiritaPrevio(String conhecimentoEspiritaPrevio) {
        this.conhecimentoEspiritaPrevio = conhecimentoEspiritaPrevio;
    }

    public String getDataAtendimento() {
        return dataAtendimento;
    }

    public void setDataAtendimento(String dataAtendimento) {
        this.dataAtendimento = dataAtendimento;
    }

    public String getDataNascimento() {
        return dataNascimento;
    }

    public void setDataNascimento(String dataNascimento) {
        this.dataNascimento = dataNascimento;
    }

    public String getDengue() {
        return dengue;
    }

    public void setDengue(String dengue) {
        this.dengue = dengue;
    }

    public String getDengueObs() {
        return dengueObs;
    }

    public void setDengueObs(String dengueObs) {
        this.dengueObs = dengueObs;
    }

    public String getDenguePeriodo() {
        return denguePeriodo;
    }

    public void setDenguePeriodo(String denguePeriodo) {
        this.denguePeriodo = denguePeriodo;
    }

    public String getDengueTipo() {
        return dengueTipo;
    }

    public void setDengueTipo(String dengueTipo) {
        this.dengueTipo = dengueTipo;
    }

    public String getDiabetes() {
        return diabetes;
    }

    public void setDiabetes(String diabetes) {
        this.diabetes = diabetes;
    }

    public String getDiabetesObs() {
        return diabetesObs;
    }

    public void setDiabetesObs(String diabetesObs) {
        this.diabetesObs = diabetesObs;
    }

    public String getDiabetesPeriodo() {
        return diabetesPeriodo;
    }

    public void setDiabetesPeriodo(String diabetesPeriodo) {
        this.diabetesPeriodo = diabetesPeriodo;
    }

    public String getDiabetesTipo() {
        return diabetesTipo;
    }

    public void setDiabetesTipo(String diabetesTipo) {
        this.diabetesTipo = diabetesTipo;
    }

    public String getDoencaAutoimune() {
        return doencaAutoimune;
    }

    public void setDoencaAutoimune(String doencaAutoimune) {
        this.doencaAutoimune = doencaAutoimune;
    }

    public String getDoencaAutoimuneObs() {
        return doencaAutoimuneObs;
    }

    public void setDoencaAutoimuneObs(String doencaAutoimuneObs) {
        this.doencaAutoimuneObs = doencaAutoimuneObs;
    }

    public String getDoencaAutoimunePeriodo() {
        return doencaAutoimunePeriodo;
    }

    public void setDoencaAutoimunePeriodo(String doencaAutoimunePeriodo) {
        this.doencaAutoimunePeriodo = doencaAutoimunePeriodo;
    }

    public String getDoencaAutoimuneTipo() {
        return doencaAutoimuneTipo;
    }

    public void setDoencaAutoimuneTipo(String doencaAutoimuneTipo) {
        this.doencaAutoimuneTipo = doencaAutoimuneTipo;
    }

    public String getDoencaEstomago() {
        return doencaEstomago;
    }

    public void setDoencaEstomago(String doencaEstomago) {
        this.doencaEstomago = doencaEstomago;
    }

    public String getDoencaEstomagoObs() {
        return doencaEstomagoObs;
    }

    public void setDoencaEstomagoObs(String doencaEstomagoObs) {
        this.doencaEstomagoObs = doencaEstomagoObs;
    }

    public String getDoencaEstomagoPeriodo() {
        return doencaEstomagoPeriodo;
    }

    public void setDoencaEstomagoPeriodo(String doencaEstomagoPeriodo) {
        this.doencaEstomagoPeriodo = doencaEstomagoPeriodo;
    }

    public String getDoencaEstomagoTipo() {
        return doencaEstomagoTipo;
    }

    public void setDoencaEstomagoTipo(String doencaEstomagoTipo) {
        this.doencaEstomagoTipo = doencaEstomagoTipo;
    }

    public String getDoencaOssos() {
        return doencaOssos;
    }

    public void setDoencaOssos(String doencaOssos) {
        this.doencaOssos = doencaOssos;
    }

    public String getDoencaOssosObs() {
        return doencaOssosObs;
    }

    public void setDoencaOssosObs(String doencaOssosObs) {
        this.doencaOssosObs = doencaOssosObs;
    }

    public String getDoencaOssosPeriodo() {
        return doencaOssosPeriodo;
    }

    public void setDoencaOssosPeriodo(String doencaOssosPeriodo) {
        this.doencaOssosPeriodo = doencaOssosPeriodo;
    }

    public String getDoencaOssosTipo() {
        return doencaOssosTipo;
    }

    public void setDoencaOssosTipo(String doencaOssosTipo) {
        this.doencaOssosTipo = doencaOssosTipo;
    }

    public String getDoencaPulmoes() {
        return doencaPulmoes;
    }

    public void setDoencaPulmoes(String doencaPulmoes) {
        this.doencaPulmoes = doencaPulmoes;
    }

    public String getDoencaPulmoesObs() {
        return doencaPulmoesObs;
    }

    public void setDoencaPulmoesObs(String doencaPulmoesObs) {
        this.doencaPulmoesObs = doencaPulmoesObs;
    }

    public String getDoencaPulmoesPeriodo() {
        return doencaPulmoesPeriodo;
    }

    public void setDoencaPulmoesPeriodo(String doencaPulmoesPeriodo) {
        this.doencaPulmoesPeriodo = doencaPulmoesPeriodo;
    }

    public String getDoencaPulmoesTipo() {
        return doencaPulmoesTipo;
    }

    public void setDoencaPulmoesTipo(String doencaPulmoesTipo) {
        this.doencaPulmoesTipo = doencaPulmoesTipo;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String getFeridasTumores() {
        return feridasTumores;
    }

    public void setFeridasTumores(String feridasTumores) {
        this.feridasTumores = feridasTumores;
    }

    public String getFeridasTumoresObs() {
        return feridasTumoresObs;
    }

    public void setFeridasTumoresObs(String feridasTumoresObs) {
        this.feridasTumoresObs = feridasTumoresObs;
    }

    public String getFeridasTumoresPeriodo() {
        return feridasTumoresPeriodo;
    }

    public void setFeridasTumoresPeriodo(String feridasTumoresPeriodo) {
        this.feridasTumoresPeriodo = feridasTumoresPeriodo;
    }

    public String getFeridasTumoresTipo() {
        return feridasTumoresTipo;
    }

    public void setFeridasTumoresTipo(String feridasTumoresTipo) {
        this.feridasTumoresTipo = feridasTumoresTipo;
    }

    public String getFonteConhecimentoEspirita() {
        return fonteConhecimentoEspirita;
    }

    public void setFonteConhecimentoEspirita(String fonteConhecimentoEspirita) {
        this.fonteConhecimentoEspirita = fonteConhecimentoEspirita;
    }

    public String getFumo() {
        return fumo;
    }

    public void setFumo(String fumo) {
        this.fumo = fumo;
    }

    public String getFumoObs() {
        return fumoObs;
    }

    public void setFumoObs(String fumoObs) {
        this.fumoObs = fumoObs;
    }

    public String getFumoPeriodo() {
        return fumoPeriodo;
    }

    public void setFumoPeriodo(String fumoPeriodo) {
        this.fumoPeriodo = fumoPeriodo;
    }

    public String getFumoTipo() {
        return fumoTipo;
    }

    public void setFumoTipo(String fumoTipo) {
        this.fumoTipo = fumoTipo;
    }

    public String getHepatite() {
        return hepatite;
    }

    public void setHepatite(String hepatite) {
        this.hepatite = hepatite;
    }

    public String getHepatiteObs() {
        return hepatiteObs;
    }

    public void setHepatiteObs(String hepatiteObs) {
        this.hepatiteObs = hepatiteObs;
    }

    public String getHepatitePeriodo() {
        return hepatitePeriodo;
    }

    public void setHepatitePeriodo(String hepatitePeriodo) {
        this.hepatitePeriodo = hepatitePeriodo;
    }

    public String getHepatiteTipo() {
        return hepatiteTipo;
    }

    public void setHepatiteTipo(String hepatiteTipo) {
        this.hepatiteTipo = hepatiteTipo;
    }

    public String getHernia() {
        return hernia;
    }

    public void setHernia(String hernia) {
        this.hernia = hernia;
    }

    public String getHerniaObs() {
        return herniaObs;
    }

    public void setHerniaObs(String herniaObs) {
        this.herniaObs = herniaObs;
    }

    public String getHerniaPeriodo() {
        return herniaPeriodo;
    }

    public void setHerniaPeriodo(String herniaPeriodo) {
        this.herniaPeriodo = herniaPeriodo;
    }

    public String getHerniaTipo() {
        return herniaTipo;
    }

    public void setHerniaTipo(String herniaTipo) {
        this.herniaTipo = herniaTipo;
    }

    public String getHipertensao() {
        return hipertensao;
    }

    public void setHipertensao(String hipertensao) {
        this.hipertensao = hipertensao;
    }

    public String getHipertensaoObs() {
        return hipertensaoObs;
    }

    public void setHipertensaoObs(String hipertensaoObs) {
        this.hipertensaoObs = hipertensaoObs;
    }

    public String getHipertensaoPeriodo() {
        return hipertensaoPeriodo;
    }

    public void setHipertensaoPeriodo(String hipertensaoPeriodo) {
        this.hipertensaoPeriodo = hipertensaoPeriodo;
    }

    public String getHipertensaoTipo() {
        return hipertensaoTipo;
    }

    public void setHipertensaoTipo(String hipertensaoTipo) {
        this.hipertensaoTipo = hipertensaoTipo;
    }

    public String getIntensidadeSintomas() {
        return intensidadeSintomas;
    }

    public void setIntensidadeSintomas(String intensidadeSintomas) {
        this.intensidadeSintomas = intensidadeSintomas;
    }

    public String getModoDesobsessao() {
        return modoDesobsessao;
    }

    public void setModoDesobsessao(String modoDesobsessao) {
        this.modoDesobsessao = modoDesobsessao;
    }

    public String getNome() {
        return nome;
    }

    public void setNome(String nome) {
        this.nome = nome;
    }

    public String getObjetivoTratamento() {
        return objetivoTratamento;
    }

    public void setObjetivoTratamento(String objetivoTratamento) {
        this.objetivoTratamento = objetivoTratamento;
    }

    public String getPacienteReincidente() {
        return pacienteReincidente;
    }

    public void setPacienteReincidente(String pacienteReincidente) {
        this.pacienteReincidente = pacienteReincidente;
    }

    public String getRemedio() {
        return remedio;
    }

    public void setRemedio(String remedio) {
        this.remedio = remedio;
    }

    public String getRemedioObs() {
        return remedioObs;
    }

    public void setRemedioObs(String remedioObs) {
        this.remedioObs = remedioObs;
    }

    public String getRemedioPeriodo() {
        return remedioPeriodo;
    }

    public void setRemedioPeriodo(String remedioPeriodo) {
        this.remedioPeriodo = remedioPeriodo;
    }

    public String getRemedioTipo() {
        return remedioTipo;
    }

    public void setRemedioTipo(String remedioTipo) {
        this.remedioTipo = remedioTipo;
    }

    public String getRua() {
        return rua;
    }

    public void setRua(String rua) {
        this.rua = rua;
    }

    public String getSetor() {
        return setor;
    }

    public void setSetor(String setor) {
        this.setor = setor;
    }

    public String getSintomas() {
        return sintomas;
    }

    public void setSintomas(String sintomas) {
        this.sintomas = sintomas;
    }

    public String getStatusTratamento() {
        return statusTratamento;
    }

    public void setStatusTratamento(String statusTratamento) {
        this.statusTratamento = statusTratamento;
    }

    public String getTelefone() {
        return telefone;
    }

    public void setTelefone(String telefone) {
        this.telefone = telefone;
    }

    public String getUsoParacetamol() {
        return usoParacetamol;
    }

    public void setUsoParacetamol(String usoParacetamol) {
        this.usoParacetamol = usoParacetamol;
    }

    public String getUsoParacetamolObs() {
        return usoParacetamolObs;
    }

    public void setUsoParacetamolObs(String usoParacetamolObs) {
        this.usoParacetamolObs = usoParacetamolObs;
    }

    public String getUsoParacetamolPeriodo() {
        return usoParacetamolPeriodo;
    }

    public void setUsoParacetamolPeriodo(String usoParacetamolPeriodo) {
        this.usoParacetamolPeriodo = usoParacetamolPeriodo;
    }

    public String getUsoParacetamolTipo() {
        return usoParacetamolTipo;
    }

    public void setUsoParacetamolTipo(String usoParacetamolTipo) {
        this.usoParacetamolTipo = usoParacetamolTipo;
    }

    public int getIdade() {
        return idade;
    }

    public void setIdade(int idade) {
        this.idade = idade;
    }

    public int getNumCasa() {
        return numCasa;
    }

    public void setNumCasa(int numCasa) {
        this.numCasa = numCasa;
    }

    @Override
    public String toString() {
        return "MedicamentoTratamentoModel{" + "idMedicamentoTratamento=" + idMedicamentoTratamento + ", acompanhamentoPaciente=" + acompanhamentoPaciente + ", alcool=" + alcool + ", alcoolObs=" + alcoolObs + ", alcoolPeriodo=" + alcoolPeriodo + ", alcoolTipo=" + alcoolTipo + ", alergias=" + alergias + ", alergiasObs=" + alergiasObs + ", alergiasPeriodo=" + alergiasPeriodo + ", alergiasTipo=" + alergiasTipo + ", atendente=" + atendente + ", bairro=" + bairro + ", cancer=" + cancer + ", cancerObs=" + cancerObs + ", cancerPeriodo=" + cancerPeriodo + ", cancerTipo=" + cancerTipo + ", cidade=" + cidade + ", conhecimentoEspiritaPrevio=" + conhecimentoEspiritaPrevio + ", dataAtendimento=" + dataAtendimento + ", dataNascimento=" + dataNascimento + ", dengue=" + dengue + ", dengueObs=" + dengueObs + ", denguePeriodo=" + denguePeriodo + ", dengueTipo=" + dengueTipo + ", diabetes=" + diabetes + ", diabetesObs=" + diabetesObs + ", diabetesPeriodo=" + diabetesPeriodo + ", diabetesTipo=" + diabetesTipo + ", doencaAutoimune=" + doencaAutoimune + ", doencaAutoimuneObs=" + doencaAutoimuneObs + ", doencaAutoimunePeriodo=" + doencaAutoimunePeriodo + ", doencaAutoimuneTipo=" + doencaAutoimuneTipo + ", doencaEstomago=" + doencaEstomago + ", doencaEstomagoObs=" + doencaEstomagoObs + ", doencaEstomagoPeriodo=" + doencaEstomagoPeriodo + ", doencaEstomagoTipo=" + doencaEstomagoTipo + ", doencaOssos=" + doencaOssos + ", doencaOssosObs=" + doencaOssosObs + ", doencaOssosPeriodo=" + doencaOssosPeriodo + ", doencaOssosTipo=" + doencaOssosTipo + ", doencaPulmoes=" + doencaPulmoes + ", doencaPulmoesObs=" + doencaPulmoesObs + ", doencaPulmoesPeriodo=" + doencaPulmoesPeriodo + ", doencaPulmoesTipo=" + doencaPulmoesTipo + ", email=" + email + ", feridasTumores=" + feridasTumores + ", feridasTumoresObs=" + feridasTumoresObs + ", feridasTumoresPeriodo=" + feridasTumoresPeriodo + ", feridasTumoresTipo=" + feridasTumoresTipo + ", fonteConhecimentoEspirita=" + fonteConhecimentoEspirita + ", fumo=" + fumo + ", fumoObs=" + fumoObs + ", fumoPeriodo=" + fumoPeriodo + ", fumoTipo=" + fumoTipo + ", hepatite=" + hepatite + ", hepatiteObs=" + hepatiteObs + ", hepatitePeriodo=" + hepatitePeriodo + ", hepatiteTipo=" + hepatiteTipo + ", hernia=" + hernia + ", herniaObs=" + herniaObs + ", herniaPeriodo=" + herniaPeriodo + ", herniaTipo=" + herniaTipo + ", hipertensao=" + hipertensao + ", hipertensaoObs=" + hipertensaoObs + ", hipertensaoPeriodo=" + hipertensaoPeriodo + ", hipertensaoTipo=" + hipertensaoTipo + ", intensidadeSintomas=" + intensidadeSintomas + ", modoDesobsessao=" + modoDesobsessao + ", nome=" + nome + ", objetivoTratamento=" + objetivoTratamento + ", pacienteReincidente=" + pacienteReincidente + ", remedio=" + remedio + ", remedioObs=" + remedioObs + ", remedioPeriodo=" + remedioPeriodo + ", remedioTipo=" + remedioTipo + ", rua=" + rua + ", setor=" + setor + ", sintomas=" + sintomas + ", statusTratamento=" + statusTratamento + ", telefone=" + telefone + ", usoParacetamol=" + usoParacetamol + ", usoParacetamolObs=" + usoParacetamolObs + ", usoParacetamolPeriodo=" + usoParacetamolPeriodo + ", usoParacetamolTipo=" + usoParacetamolTipo + ", idade=" + idade + ", numCasa=" + numCasa + '}';
    }

    
}
